...
SELECT LTRIM(call_id, ‘MIS’)
FROM course
WHERE course_name LIKE ‘%Intro%’;
SELECT RTRIM(call_id, ‘101’)
FROM course
WHERE course_name LIKE '%Intro%';


SELECT INITCAP(bldg_code) FROM location
WHERE loc_id = 1;

SELECT LENGTH(term_desc) FROM term
WHERE term_id = 1;


SELECT LPAD(term_desc, 12, '*') FROM term
WHERE term_id = 1;

SELECT REPLACE(term_desc, '2005', '1995')
FROM term
WHERE term_id = 1;


SELECT term_desc FROM term
WHERE term_id = 1;

SELECT SUBSTR(term_desc, 1, 4) AS first_four FROM term
WHERE term_id = 1;


SELECT UPPER(term_desc) AS upper_case FROM term
WHERE term_id = 1;

SELECT LOWER(term_desc) AS lower_case FROM term
WHERE term_id = 1;
